...
public class HighPerformance {
	...
	@RequestMapping(value = "/highOtdFpyPpm", method = RequestMethod.POST)
		...
		try {
			if (!criteria.isEmpty() && !year.isEmpty()) {
				...
				if (criteria.equalsIgnoreCase("otd")) {
							...
							if (perfList.size() != 0) {
								...
								if (otdPF != null) {
									if (type.equalsIgnoreCase("undefined")) {
										...
										otd = otdServices.getHighPerForOtd(year, otdPF, otdSuppCodeForD,
												otdSuppTypeForD);
										
										arrli.addAll(otd);
										//logger.info("OTD LIST SIZE===   "+arrli.size());

									} else {
										otd = otdServices.getHighPerForOtd(year, otdPF, otdSuppCodeForD, type);
										arrli.addAll(otd);
									}
								} else {
					...
					return new RestResponse(1, "Otd high performance", arrli);
				}
							...
							if (perfList.size() != 0) {
								...
								if (fpyPF != null) {
									if (type.equalsIgnoreCase("undefined")) {
										...
										fpy = fpyServices.getHighPerForFpy(year, fpyPF, fpySuppCodeF, fpySuppTypeF);
										arrli.addAll(fpy);
									} else {
										fpy = fpyServices.getHighPerForFpy(year, fpyPF, fpySuppCodeF, type);
										arrli.addAll(fpy);
									}
								} else {
					...
					return new RestResponse(1, "Fpy high performance", arrli);
				}
							...
							if (perfList.size() != 0) {
								...
								if (ppmPF != null) {
									if (type.equalsIgnoreCase("undefined")) {
										...
										ppm = ppmServices.getHighPerForPpm(year, ppmPF, ppmSuppCodeF, ppmSuppTypeF);
										arrli.addAll(ppm);

									} else {
										ppm = ppmServices.getHighPerForPpm(year, ppmPF, ppmSuppCodeF, type);
										arrli.addAll(ppm);
									}
								} else {
									//logger.info("null value found");
					...
					return new RestResponse(1, "PPm high performance", arrli);
				} else {
